...
> R:¨carrito init¨ pone la cantidad de elementos del carrito en 0, es decir, lo vacía. Es importante llamarla antes de usar el carrito porque permite utilizar el carrito vacío al comenzar una compra, sin tener productos de otro carrito.
...
> R:Falla. Esperaba el valor "700" y obtuvo el valor "350"
...
> R:Para calcular el total correctamente, debería multiplicar c->items[i].precio por "cantidad"
...
> R:EL test falla. "carrito_agregar" devuelve 1, agregando el elemento.
...
> R:La líneas 29 y 30 tienen `#####`, es decir, no se ejecutaron. Esto se debe a que no se utiliza "
> R:La líneas 29 y 30 tienen `#####`, es decir, no se ejecutaron. Esto se debe a que no se utiliza "carrito_descuento" en los tests unitarios. 
...
> R:La diferencia entre un test unitario y uno de integración es que el primero sirve para verificar la validez de funciones individuales, mientras que el segundo verifica la validez de las funciones como un conjunto. El primero en detectar el bug de `carrito_total` fue el test unitario.
...
> R:Es peligroso en C porque no se controla el espacio del array en tiempo de ejecución como sí se hace en Python o en Java.
...
> R:Es mejor encontrar los bugs mediante tests porque se trata de probar el código y hallar bugs mediante compilación y ejecución del programa, no analizando el código visualmente (lo cual podría demandar mucho tiempo)
...
> R:No se detectó el bug en `test_total_precio_unitario` porque, al ser la cantidad de un solo producto, el precio total del carrito es igual al del producto. Por otro lado, en `test_total_con_cantidad`, al tener más de un producto, el precio total del carrito difiere y permite visualizar el bug.
